...
function CardGame() {
  ...
  const [asciiArt, setAsciiArt] = useState(""); // New state for ASCII art
...
`;
    ...
    try {
      ...
      if (rawText) {
        ...
        // Generate ASCII art based on the story
        const asciiPrompt = `
Given the following story excerpt, create an ASCII art comic that visually represents the scene described. The art should be simple and fit within a small rectangular frame. The story excerpt is as follows:

"${story}"

ASCII art response:
`;
        const asciiResponse = await generateContent(asciiPrompt);
        if (asciiResponse) {
          setAsciiArt(asciiResponse); // Set the generated ASCII art
        }
      } else {
      ...
      <div className="story-container">
        ...
        {asciiArt && (
          <pre className="ascii-art">
            {asciiArt} {/* Display the ASCII art */}
          </pre>
        )}
